const ALLOWED_REDIRECT_PATTERNS = [
  // Chrome / Edge (Chromium) — chrome-extension://<id>/oauth-callback
  /^chrome-extension:\/\/[a-p]{32}\/oauth-callback(\?.*)?$/,
  // Chrome / Edge (Chromium) — https://<id>.chromiumapp.org/oauth-callback
  /^https:\/\/[a-p]{32}\.chromiumapp\.org\/oauth-callback(\?.*)?$/,
  // Firefox — https://<sha1-hex>.extensions.allizom.org/oauth-callback
  // Subdomain is the SHA-1 hex digest (40 chars) of the extension ID, computed by Firefox
  /^https:\/\/[0-9a-f]{40}\.extensions\.allizom\.org\/oauth-callback(\?.*)?$/,
];
