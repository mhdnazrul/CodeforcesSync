...
The service worker calls `browserApi.identity.launchWebAuthFlow()` with a URL pointing to the Vercel broker's `/api/oauth/authorize` endpoint.
...
- The `state` and `verifier` are stored in `browserApi.storage.session` (ephemeral, cleared on browser restart).
...
3. Redirects to the extension's redirect URI (determined by `browserApi.identity.getRedirectURL()`) with token, username, and state as query parameters.
...
The token is now stored in `storage.local` — accessible only to the extension's own origin.
...
| **Token delivery** | Token delivered via the browser's `identity.launchWebAuthFlow()` redirect — never hits a network server the extension doesn't control. |
...
|-------|----------|-------------|
| `redirect_uri` | Yes | The extension's redirect URI. Chrome/Edge: `chrome-extension://<id>/oauth-callback`. Firefox: `https://<uuid>.extensions.allizom.org/oauth-callback`. Generated by `browserApi.identity.getRedirectURL()`. |
...
**Response:** 307 redirect to the extension's `redirect_uri` with `token`, `username`, and `state` query parameters. In Chrome/Edge the URI scheme is `chrome-extension://`; in Firefox it is `https://<uuid>.extensions.allizom.org/`.
...
- **Compromised broker** — An attacker who controls the Vercel deployment could swap the token during the callback. Mitigation: the final redirect goes to the extension's own origin (the `identity.getRedirectURL()` scheme), which the browser intercepts locally; the attacker never sees the delivered token. However, a malicious broker could return a different token or deny the flow. This risk is accepted because the broker is stateless and the Vercel deployment is controlled by the project maintainer.

- **Compromised extension** — The token is stored in `storage.local`, accessible only to the extension's own origin. Other extensions cannot read it.

- **Network eavesdropping** — All broker communications are HTTPS. The token is never transmitted in URLs to external servers — only to the local extension redirect URL intercepted by the browser.
